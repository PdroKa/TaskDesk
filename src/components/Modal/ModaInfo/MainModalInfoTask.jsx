...
import { SwitchTabDetailsModal } from './detailSwitchTab/index'

import anexarIcon from '../../../image/AnexarDoc.svg'
import linkIcon from '../../../image/anexarLink.svg'

import { ModalAdminOpen } from './ModalOpen/OpenForAdmin';

export function ModalInfoTask({ isOpen, data }) {

    return (
        <>
            {data.postId == 100 && <ModalAdminOpen isOpen={isOpen} data={data} />}
            {data.postId == 1 && <ModalAdminOpen isOpen={isOpen} data={data} />}
            {data.postId == 2 && <ModalAdminPending isOpen={isOpen} data={data} />}
            {data.postId == 3 && <ModalAdminFinish />}
            {data.postId == 4 && <ModalTaskDisable />}
        </>
    )
}
...
const ModalAdminPending = ({ id = "modalAdminOpen", isOpen, data }) => {
